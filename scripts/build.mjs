...
function filters(options) { return `<div class="filter-bar"><label><span>分類</span><select data-filter><option value="">全部分類</option>${options.map(x=>`<option>${esc(x)}</option>`).join('')}</select></label><button class="button" type="button" data-clear>清除篩選</button><output data-result-count></output></div>`; }
function empty() { return `<div class="empty-state" data-empty hidden><span class="seal">空</span><h2>沒有符合條件的資料</h2><p>改選其他分類，或清除篩選後再看看。</p><button class="button" type="button" data-clear>清除篩選</button></div>`; }
...
function shopping(data) {
  ...
  return layout({title:'購物與商店',eyebrow:'쇼핑 · Shopping',active:'shopping',body:`<section class="shopping-intro"><div><p class="eyebrow">LIST 01</p><h2>去哪裡逛</h2><p>這些是行程裡已有的商店線索，分店未明時不猜地址。</p></div><div><p class="eyebrow">LIST 02</p><h2>要買什麼</h2><p>商品、規格與數量會在你提供清單後加入。</p></div></section><section data-filter-scope>${filters([...new Set(data.stores.flatMap(x=>[x.area,x.category]).filter(Boolean))])}<div class="catalog-grid">${stores}</div>${empty()}</section><section><div class="section-title"><div><p class="eyebrow">BUY LIST</p><h2>商品清單</h2></div></div>${products}</section>`});
}
