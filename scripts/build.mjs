...
function food(data) {
  const card=(x)=>{const dates=scheduledDates(data,x.id);const direction=x.image?null:x.direction.slice(0,1);const notes=publicRestaurantNotes(x);const fromScreenshot=Boolean(x.image); return `<article id="${x.id}" class="catalog-card food-card"><div class="card-top">${chips([direction,x.area,...x.foodTypes.slice(0,1)])}</div>${referenceImage(x.image,x.nameZh)}<h2>${esc(x.nameZh)}</h2>${x.nameKo?`<p class="ko" lang="ko">${esc(x.nameKo)}</p>`:''}${dates.length?`<a class="text-link scheduled" href="../itinerary/?day=${esc(data.days.find(day=>day.events.some(e=>e.placeId===x.id)).id)}">已排入 ${esc(dates.join('、'))} →</a>`:''}${fromScreenshot?'':`<div class="hours"><small>營業與休業提醒</small><p>${lines(x.rawHours)}</p></div>`}<details><summary>餐點與備註</summary>${x.recommendedDishes.length?`<h3>餐點</h3><ul>${x.recommendedDishes.map(i=>`<li>${esc(i)}</li>`).join('')}</ul>`:''}${notes.length?`<h3>注意事項</h3><ul>${notes.map(i=>`<li>${esc(i)}</li>`).join('')}</ul>`:''}${fromScreenshot?'':`<p><b>確認狀態：</b>${esc(x.identityStatus)}</p>`}</details></article>`};
...
function shopping(data) {
  ...
  const products=data.shoppingItems.length?`<div class="catalog-grid product-grid">${data.shoppingItems.map(x=>`<article id="${esc(x.id)}" class="catalog-card product-card">${referenceImage(x.image,x.name)}<h3>${esc(x.name)}</h3>${x.referencePrice?`<p class="product-price">${esc(x.referencePrice)}</p>`:''}</article>`).join('')}</div>`:`<div class="empty-state product-empty"><span class="seal">待</span><h2>購物清單待補</h2><p>之後會加入商品、規格與購買地點。目前沒有虛構商品、價格或庫存。</p></div>`;
  return layout({title:'購物與商店',eyebrow:'쇼핑 · Shopping',active:'shopping',body:`<section class="shopping-intro"><div><p class="eyebrow">LIST 01</p><h2>去哪裡逛</h2><p>這些是行程裡已有的商店線索，分店未明時不猜地址。</p></div><div><p class="eyebrow">LIST 02</p><h2>要買什麼</h2><p>已依你提供的截圖整理 ${data.shoppingItems.length} 項商品，點圖片可查看原始截圖。</p></div></section><div class="catalog-grid">${stores}</div><section><div class="section-title"><div><p class="eyebrow">BUY LIST</p><h2>購物清單</h2></div></div>${products}</section>`});
}
